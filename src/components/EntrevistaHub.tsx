...
import { 
  ...
  Send, Briefcase, FileSignature, Mail, CheckSquare, PenTool
} from 'lucide-react';
      ...
      <div className="relative">
        {isLoading ? (
          ...
          <>
            ...
            />
            {isLocked && (
              <button type="button" onClick={onUnlock} className="absolute right-3 top-1/2 -translate-y-1/2 p-2 bg-white/10 rounded-lg text-white/40 hover:text-white transition-all z-10">
                <RotateCcw size={14}/>
              </button>
            )}
          </>
...
export default function EntrevistaHub() {
  ...
  // Monitor de estatus en tiempo real para el Paso 4
  useEffect(() => {
  ...
  const handleFinalizarAuditoria = async () => {
    if (!asesorNombre || sigCanvasAsesor.current?.isEmpty()) {
      alert("El asesor debe firmar la hoja de diagnóstico antes de enviarla.");
      return;
    }
    setIsProcessing(true);
    const firmaAsesorBase64 = sigCanvasAsesor.current?.getTrimmedCanvas().toDataURL('image/png');
    try {
      const res = await callGAS('FINALIZE_AUDIT', {
        clienteId: data.id,
        asesor: asesorNombre,
        firmaAsesor: firmaAsesorBase64,
        dictamen: hojaServicio.notasDiagnostico,
        servicios: hojaServicio.servicios.join(', ') + (hojaServicio.otroServicioTexto ? `, ${hojaServicio.otroServicioTexto}` : ''),
          ...
          <div className="max-w-4xl mx-auto py-10">
            <div className="bg-white p-10 rounded-[48px] shadow-2xl border border-slate-100 space-y-8">
               ...
               <div className="flex flex-col md:flex-row gap-4">
                 <button type="button" onClick={handleSearch} disabled={isSearching} className="flex-1 py-6 bg-[#003366] text-white rounded-[24px] font-black uppercase tracking-widest hover:bg-[#002244] transition-all flex items-center justify-center gap-3 shadow-xl shadow-blue-100">
                   {isSearching ? <Loader2 className="animate-spin" size={20} /> : <Search size={20} />} Verificar Ecosistema
                 </button>
                 <button type="button" onClick={handleNewClient} className="flex-1 py-6 bg-[#DAA520] text-[#003366] rounded-[24px] font-black uppercase tracking-widest hover:bg-[#c4941d] transition-all flex items-center justify-center gap-3 shadow-xl shadow-yellow-100">
                        ...
                        {[
                          ...
                          return (
                            <div key={doc.id} className={cn("p-6 rounded-[32px] border-2 transition-all flex flex-col items-center text-center gap-4 relative", exists ? "bg-emerald-50 border-emerald-100" : "bg-slate-50 border-slate-100 hover:border-[#003366]")}>
                              {exists && <button type="button" onClick={() => handleDeleteFile(doc.id)} className="absolute top-4 right-4 p-2 text-red-400 hover:bg-red-50 rounded-full z-20"><Trash2 size={18}/></button>}
                              ...
                              <div><p className="text-sm font-black uppercase leading-none text-slate-900">{doc.label}</p><p className="text-[9px] font-bold text-slate-400 uppercase mt-1">{doc.sub}</p></div>
                              <button type="button" onClick={() => { activeUploadRef.current = doc.id; fileInputRef.current?.click(); }} className={cn("w-full py-3 rounded-xl font-black text-[10px] uppercase shadow-sm transition-all", exists ? "bg-white text-emerald-600 border border-emerald-100" : "bg-[#003366] text-white")}>
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                       <div className="space-y-6">
                         ...
                         <div className="space-y-4 p-5 bg-slate-50 rounded-3xl border border-slate-200">
                            ...
                            <div className="grid grid-cols-2 gap-3 mb-4">
                                <button type="button" onClick={() => setHojaServicio({...hojaServicio, universo: 'U1'})} className={cn("p-4 border-2 rounded-2xl font-bold text-sm transition-all", hojaServicio.universo === 'U1' ? "border-[#003366] bg-white text-[#003366] shadow-md" : "bg-transparent text-slate-400 border-transparent")}>U1 (Única Vez)</button>
                                <button type="button" onClick={() => setHojaServicio({...hojaServicio, universo: 'U2'})} className={cn("p-4 border-2 rounded-2xl font-bold text-sm transition-all", hojaServicio.universo === 'U2' ? "border-[#DAA520] bg-white text-[#DAA520] shadow-md" : "bg-transparent text-slate-400 border-transparent")}>U2 (Recurrente)</button>
                            </div>
                            <input type="number" value={hojaServicio.honorariosAcordados || ''} onChange={(e) => setHojaServicio({...hojaServicio, honorariosAcordados: Number(e.target.value)})} placeholder="Honorarios ($)" className="w-full p-4 bg-white border-2 border-slate-100 rounded-2xl font-black text-lg focus:border-[#DAA520] outline-none"/>
                         </div>
                       </div>
                       <div className="bg-slate-900 rounded-[32px] p-6 flex flex-col h-[550px] border border-slate-800 shadow-2xl relative">
                          <div className="flex items-center justify-between border-b border-slate-800 pb-4 mb-4">
                             <div className="flex items-center gap-3"><MessageSquare className="text-emerald-400" size={20} /><h4 className="text-sm font-black text-white uppercase">IA Co-Pilot Consultivo</h4></div>
                             <button type="button" onClick={redactarDiagnosticoIA} disabled={isAiDrafting} className="bg-emerald-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase flex items-center gap-2">
                               {isAiDrafting ? <Loader2 size={12} className="animate-spin"/> : <Sparkles size={12}/>} Redactar con IA
                             </button>
                          </div>
                          <textarea value={iaContext} onChange={(e) => setIaContext(e.target.value)} className="w-full bg-slate-800 text-white text-xs p-4 rounded-2xl mb-4 h-28 outline-none border border-slate-700" placeholder="Instrucciones para Gemini..."/>
                          <textarea value={hojaServicio.notasDiagnostico} onChange={(e) => setHojaServicio({...hojaServicio, notasDiagnostico: e.target.value})} className="flex-1 w-full bg-slate-800 text-emerald-50 border border-slate-700 rounded-2xl p-4 text-sm font-medium leading-relaxed outline-none focus:border-emerald-500 resize-none custom-scrollbar" placeholder="El dictamen técnico final..."/>
                       </div>
                     </div>
                  </div>
                )}
                  ...
                  <div className="bg-white p-12 rounded-[48px] shadow-2xl border border-slate-100 space-y-10">
                    ...
                    <div className="bg-slate-50 p-8 rounded-3xl border border-slate-200 h-96 overflow-y-auto text-xs font-mono text-slate-700 leading-relaxed shadow-inner custom-scrollbar">
                      <div className="space-y-6">
                        ...
                        <p>BAKSO, S.C. emite el siguiente dictamen para el <strong>C. {data.nombre || '[NOMBRE COMPLETO]'}</strong>, identificado con CURP <strong>{data.curp || '[CURP]'}</strong> y RFC <strong>{data.rfc || '[RFC]'}</strong>.</p>
                        <div className="bg-white p-6 rounded-2xl border border-slate-200 italic text-slate-900 leading-normal whitespace-pre-wrap">{hojaServicio.notasDiagnostico || 'Diagnóstico pendiente...'}</div>
                        <div className="border-t border-slate-200 pt-6">
                          <p className="font-black text-slate-900 uppercase">Fundamentación Legal:</p>
                          <p className="mt-2">Este dictamen constituye el anexo técnico del Contrato de Prestación de Servicios Profesionales. La firma electrónica estampada en el portal externo tiene plena validez jurídica conforme al <strong>Artículo 1803 del Código Civil Federal</strong>.</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 border-t border-slate-100 pt-10">
                       <div className="space-y-4">
                          ...
                          <button type="button" onClick={handleFinalizarAuditoria} className="w-full py-4 bg-[#003366] text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg hover:bg-slate-800 transition-all">Certificar Diagnóstico</button>
                       </div>
                       <div className="space-y-4 flex flex-col justify-center">
                          <div className="bg-slate-50 p-6 rounded-3xl border border-slate-200 text-center space-y-6">
                             <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Generar Firma Externa del Cliente</p>
                             <div className="flex gap-2">
                                <button type="button" onClick={() => {
                                    ...
                                    const text = `Hola ${data.nombre}, para formalizar tu expediente digital de SOCIAL PUSH ingresando aquí para tu firma y selfie: ${link}`;
                                    window.open(`https://wa.me/52${data.whatsapp?.replace(/\D/g, '')}?text=${encodeURIComponent(text)}`, '_blank');
                                }} className="flex-1 py-5 bg-[#25D366] text-white rounded-2xl font-black uppercase text-[10px] flex items-center justify-center gap-2 shadow-xl hover:scale-[1.02] transition-all shadow-emerald-100">WhatsApp <Smartphone size={16}/></button>
                                <button type="button" onClick={() => {
                                    const tipoDoc = !!data.contratourl ? 'DIAGNOSTICO' : 'CONTRATO_Y_DIAGNOSTICO';
                                    const link = `${window.location.origin}/firma/${data.id}?tipoDoc=${tipoDoc}`;
                                    const body = `Hola ${data.nombre},\n\nPara formalizar tu Hoja de Diagnóstico y Servicios de SOCIAL PUSH, ingresa al siguiente enlace:\n\n${link}`;
                                    window.location.href = `mailto:${data.email}?subject=Firma de Diagnóstico - SOCIAL PUSH&body=${encodeURIComponent(body)}`;
                                }} className="flex-1 py-5 bg-[#003366] text-white rounded-2xl font-black uppercase text-[10px] flex items-center justify-center gap-2 shadow-xl hover:scale-[1.02] transition-all shadow-blue-100">Email <Mail size={16}/></button>
                             </div>
                          </div>
                       </div>
                    </div>
             ...
             <div className="lg:col-span-4">
                <div className="bg-[#003366] p-8 rounded-[48px] text-white shadow-2xl space-y-8 sticky top-28">
                   ...
                   <div className="space-y-5">
                      <div className={cn("p-4 rounded-2xl flex items-center justify-between border animate-in fade-in duration-500", data.estatusfirma === 'FIRMADO' ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" : "bg-amber-500/10 border-amber-500/20 text-amber-400")}>
                        <div className="flex items-center gap-3">
                           {data.estatusfirma === 'FIRMADO' ? <CheckCircle size={20} /> : <Clock size={20} />}
                           <span className="text-[10px] font-black uppercase tracking-widest">{data.estatusfirma === 'FIRMADO' ? 'Auditado y Cerrado ✅' : 'Esperando Firma ⏳'}</span>
                        </div>
                        <button type="button" onClick={() => getGASData('GET_CLIENTE_STATUS', { curp: data.id }).then(res => res?.data && updateData({ estatusfirma: res.data.estatusfirma }))} className="p-1 hover:bg-white/10 rounded-full transition-all"><RefreshCcw size={14} /></button>
                      </div>
                      ...
                      <div className={cn("p-4 rounded-2xl flex items-center gap-3 border animate-in zoom-in duration-300", nivelCerteza === 'Alto' ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" : "bg-amber-500/10 border-amber-500/20 text-amber-400")}>
                        {nivelCerteza === 'Alto' ? <ShieldCheck size={20} /> : <AlertTriangle size={20} />}
                        <span className="text-[10px] font-black uppercase tracking-widest">Certeza Jurídica: {nivelCerteza}</span>
                      </div>
                      ...
                      <AuditoriaInput label="Semanas Extra" value={data.semanasExtra} onChange={(v:any)=>updateData({semanasExtra: v})} />
                   ...
                   {activeStep === 3 && (
                     <button type="button" onClick={() => setActiveStep(4)} disabled={isProcessing} className="w-full py-5 bg-[#DAA520] text-[#003366] rounded-[24px] font-black uppercase shadow-xl hover:bg-[#c9961d] active:scale-95 transition-all flex items-center justify-center gap-3">
                        Continuar a Certificación <ArrowRight size={18} />
                     </button>
                   )}
                </div>
             </div>
      ...
      <AnimatePresence>
         {data.estatusfirma === 'FIRMADO' && (
            <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-[100] bg-[#003366]/95 flex items-center justify-center p-10 text-center flex-col gap-6 backdrop-blur-md">
               <div className="bg-[#DAA520] p-8 rounded-full shadow-[0_0_50px_rgba(218,165,32,0.3)]"><CheckCircle size={80} className="text-[#003366] animate-bounce"/></div>
               <h2 className="text-5xl font-black text-white uppercase tracking-tighter">¡Expediente Formalizado!</h2>
               <p className="text-xl text-emerald-400 font-bold">Diagnóstico certificado y guardado en Drive de 2TB</p>
               <button type="button" onClick={() => {setData({id: undefined}); setActiveStep(1);}} className="mt-8 px-10 py-4 bg-white text-[#003366] rounded-full font-black uppercase tracking-widest hover:bg-slate-100 transition-all">Cerrar y Regresar al Maestro</button>
            </motion.div>
         )}
      </AnimatePresence>
    </div>
